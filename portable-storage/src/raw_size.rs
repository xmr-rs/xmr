...
pub fn read<T: ByteOrder, B: Buf>(buf: &mut B) -> Result<usize> {
    ensure_eob!(buf, 1);
    ...
    match mark {
        ...
        PORTABLE_RAW_SIZE_MARK_WORD => {
            ensure_eob!(buf, 2);
            Ok((buf.get_u16::<T>() >> 2) as usize)
        }
        PORTABLE_RAW_SIZE_MARK_DWORD => {
            ensure_eob!(buf, 4);
            Ok((buf.get_u32::<T>() >> 2) as usize)
        }
        PORTABLE_RAW_SIZE_MARK_INT64 => {
            ensure_eob!(buf, 8);
            Ok((buf.get_u64::<T>() >> 2) as usize)
        }
        _ => unreachable!(),
    }
}

pub fn write<T: ByteOrder>(buf: &mut BytesMut, val: usize) {
    if val <= 63 {
        buf.reserve(1);
        buf.put_u8(((val as u8) << 2) | PORTABLE_RAW_SIZE_MARK_BYTE);
    } else if val <= 16383 {
        buf.reserve(2);
        buf.put_u16::<T>(((val as u16) << 2) | PORTABLE_RAW_SIZE_MARK_WORD as u16);
    } else if val <= 1073741823 {
        buf.reserve(4);
        buf.put_u32::<T>(((val as u32) << 2) | PORTABLE_RAW_SIZE_MARK_DWORD as u32);
    } else if val as u64 <= 4611686018427387903 {
        buf.reserve(8);
        buf.put_u64::<T>(((val as u64) << 2) | PORTABLE_RAW_SIZE_MARK_INT64 as u64);
    } else {
